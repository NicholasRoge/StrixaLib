...
import com.strixa.net.ClientSocket.DataReceivedListener;
import com.strixa.net.ClientSocket.DataSendListener;
...
public class ClientSocket{
     ...
     * @author Nicholas Rogé
     */
    public static class NotConnectedException extends Throwable{
    ...
    public static interface DataSendListener{
         ...
         */
        public boolean onDataSend(ClientSocket client,Object data);
    }
    ...
    public static interface SignalReceivedListener{
         ...
         */
        public void onSignalReceived(ClientSocket client,Signal signal);
    }
    ...
    public static interface DataReceivedListener{
         ...
         */
        public void onDataReceived(ClientSocket client,Object data);
    }
     ...
     */
    public ClientSocket(String host_address,int port,boolean require_server_sync){
    ...
    protected void _connect() throws IOException{
        ...
        new Thread(new Runnable(){
            @Override public void run(){
                try{
                    ClientSocket.this.__input=new ObjectInputStream(ClientSocket.this.__socket.getInputStream());
                }catch(IOException e){
                    System.out.print("Could not open the write stream on this client.  Cause:\n    "+e.getMessage()+"\n");
                    if(ClientSocket.this.__verbose){
                        e.printStackTrace();
                    }
                }
            }
     ...
     * Disconnects this object from the server.  This method should ALWAYS be called when the connection will no longer be used.
     * 
     * @param send_connection_close_notice Unless you are acting as the server, this should always be true.  In most cases, it is sufficient to simply use the {@link ClientSocket#disconnect()} method.
     */
    ...
    protected void _onSignalReceived(ClientSocket client,Signal signal){
        ...
        if(ensure_arrival){
            ...
            while(true){
                ...
                while(true){
                    ...
                    if((System.currentTimeMillis()-wait_begin_time)>ClientSocket.ENSURED_DATA_RECEIPT_TIMEOUT_PERIOD){
                        retry_count++;
                        
                        if(retry_count>ClientSocket.MAXIMUM_SEND_RETRY_COUNT){
                            return false;
                        }
    ...
    protected void _startMessageListener(){
        ...
        this.__message_listener=new Thread(new Runnable(){
            @Override public void run(){
                ...
                while(true){
                    try{
                            data=ClientSocket.this.__input.readObject();
                            if(ClientSocket.this.__verbose){
                                System.out.print("Data recieved:  "+data.toString()+"\n");
                            }
    
                            if(data instanceof Signal){
                                ClientSocket.this._broadcastSignalReceived((Signal)data);
                            }else{
                                ClientSocket.this._broadcastDataReceived(data);
                            }
                            
                            ClientSocket.this.send(new Signals.Ack(data),false);
